...
router.put("/users/:userId/status", async (req, res, next) => {
  try {
    ...
    const affectedRows = await userModel.updateUserStatus(userId, isActive);
    if (affectedRows === 0) {
      return next(new AppError("User not found or status already set.", 404));
    }
    logger.info(
...
router.post("/sellers", async (req, res, next) => {
  ...
  try {
    ...
    const sellerId = await sellerModel.createSeller(userId, {
      shop_name: shopName,
    ...
    logger.info(
      `Admin ${req.user.userId} created seller ${sellerId} (${shopName}).`
    );
    res.status(201).json({
      status: "success",
...
router.put("/sellers/:sellerId", async (req, res, next) => {
  try {
    ...
    const updateData = req.body; // Can contain any field from sellerModel.updateSellerProfile

    if (Object.keys(updateData).length === 0) {
      return next(new AppError("No fields provided for seller update.", 400));
    }
    ...
    if (affectedRows === 0) {
      return next(new AppError("Seller not found or no changes made.", 404));
    }
...
router.put("/sellers/:sellerId/status", async (req, res, next) => {
  try {
    ...
    if (affectedRows === 0) {
      return next(new AppError("Seller not found or status already set.", 404));
    }
    logger.info(
...
router.get("/statistics", async (req, res, next) => {
  try {
    ...
    userRoleCounts.forEach((row) => {
      ...
      if (row.role_name === "SELLER") stats.totalSellers = row.count; // Total users with seller role
    });
